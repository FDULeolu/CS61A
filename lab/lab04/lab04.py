...
def reverse_recursive(lst):
    ...
    left_reverse = reverse_recursive(left_half)
    right_reverse = reverse_recursive(right_half)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    target_city = make_city('TargetCity', lat, lon)
...
def add_chars(w1, w2):
    ...
    True
    """
    def compare_chars(w1, w2, result):
        if len(w1) == 0:
            return result + w2
        if w1[0] == w2[0]:
            return compare_chars(w1[1:], w2[1:], result)
        else:
            return compare_chars(w1, w2[1:], result + w2[0])
    
    return compare_chars(w1, w2, '')
